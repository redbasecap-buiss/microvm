...
pub fn generate_dtb_smp(
    ...
    let isa_str = "rv64imafdcvsu_zicsr_zifencei_zicbom_zicboz_zicbop_zicond_zihintpause_zawrs_zacas_zabha_zfa_zfh_zimop_zcmop_zba_zbb_zbs_zbc_zbkb_zbkc_zbkx_zknd_zkne_zknh_zkr_zve32f_zve64f_zve64d_zvbb_zvkg_zvkned_zvknhb_zvksed_zvksh_zvl128b_sstc_zicntr_zihpm_svinval_svnapot_svpbmt_svadu_smstateen_ssstateen_sscofpmf_smcntrpmf_sdtrig";
    ...
    b.begin_node(&format!("virtio_mmio@{:x}", memory::VIRTIO4_BASE));
    b.prop_str("compatible", "virtio,mmio");
    b.prop_u32_array(
        "reg",
        &[
            (memory::VIRTIO4_BASE >> 32) as u32,
            memory::VIRTIO4_BASE as u32,
            0,
            memory::VIRTIO4_SIZE as u32,
        ],
    );
    b.prop_u32_array("interrupts", &[14]);
    b.prop_u32("interrupt-parent", plic_phandle);
    b.end_node();

    // VirtIO Input
    b.begin_node(&format!("virtio_mmio@{:x}", memory::VIRTIO5_BASE));
    b.prop_str("compatible", "virtio,mmio");
    b.prop_u32_array(
        "reg",
        &[
            (memory::VIRTIO5_BASE >> 32) as u32,
            memory::VIRTIO5_BASE as u32,
            0,
            memory::VIRTIO5_SIZE as u32,
        ],
    );
    b.prop_u32_array("interrupts", &[15]);
    b.prop_u32("interrupt-parent", plic_phandle);
    b.end_node();

    // VirtIO Balloon
    b.begin_node(&format!("virtio_mmio@{:x}", memory::VIRTIO6_BASE));
    b.prop_str("compatible", "virtio,mmio");
    b.prop_u32_array(
        "reg",
        &[
            (memory::VIRTIO6_BASE >> 32) as u32,
            memory::VIRTIO6_BASE as u32,
            0,
            memory::VIRTIO6_SIZE as u32,
        ],
    );
    b.prop_u32_array("interrupts", &[16]);
    b.prop_u32("interrupt-parent", plic_phandle);
    b.end_node();
    ...
    // VirtIO vsock
    b.begin_node(&format!("virtio_mmio@{:x}", memory::VIRTIO8_BASE));
    b.prop_str("compatible", "virtio,mmio");
    b.prop_u32_array(
        "reg",
        &[
            (memory::VIRTIO8_BASE >> 32) as u32,
            memory::VIRTIO8_BASE as u32,
            0,
            memory::VIRTIO8_SIZE as u32,
        ],
    );
    b.prop_u32_array("interrupts", &[18]);
    b.prop_u32("interrupt-parent", plic_phandle);
    b.end_node();
